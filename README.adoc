...
Commentaires éventuels des étudiants : #Access token : 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#
...
Le code est structuré en trois couches différentes : base de données, code,  métier.
Le problème est que l’application est dépendente de la technologie utilisée.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Le code métier ce situe dans le dossier src/main/java/com/acmetodolist plus précisément dans le fichier TodoListController

*Cette règle est-elle facilement testable par un test unitaire ?*
La règle n’est pas facilement testable car elle est privé.
...
Le repository n’est pas initialiser. La meilleur façon de l’injecter est de le faire de façons automatique avec @Inject
...
Une architecture hexagonale permet de séparer les parties code métier des données. Si une des deux est modifiée cela n’impactera pas l'autre.
Cela facilite la maintenance du code et la stabilité.
